...
void SurfaceControl::init(const int totalWayPoints_in, double *wayPoints_in, int navigateDelay_in) //initializes
{
...
void SurfaceControl::navigate(xy_state_t *state, gps_state_t *gps_state_p, int currentTime_in)
{
  ...
  if (gps_state_p->num_sat >= N_SATS_THRESHOLD)
  {
    ...
    //if (currentWayPoint == totalWayPoints)
      //return; // stops motors at final surface point
...
String SurfaceControl::printWaypointUpdate(void)
{
  ...
  else
  {
    ...
    wayPointUpdate += String(currentWayPoint);
    //wayPointUpdate += "X_init:";
    //wayPointUpdate += String(x_init);
    //wayPointUpdate += "Y_init:";
    //wayPointUpdate += String(y_init);
    //wayPointUpdate += "uL=";
    //wayPointUpdate += String(uL);
    //wayPointUpdate += "uR=";
    //wayPointUpdate += String(uR);
    wayPointUpdate += "Current x:";
    wayPointUpdate += String(x);
    wayPointUpdate += "Current y:";
    wayPointUpdate += String(y);
